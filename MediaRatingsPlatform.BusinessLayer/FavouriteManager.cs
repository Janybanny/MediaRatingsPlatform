...
public class FavouriteManager(IRepositoryFactory database) : IFavouriteManager {
    ...
    public void Favourite(Favourite favourite) {
        ...
        if (database.CreateMediaRepository().GetMedia(new Media() { Id = favourite.MediaId }) == null) {
            throw new ApiItemDoesNotExistException();
        }
    ...
    public List<Media> GetFavourites(User user) {
        ...
        foreach (var favourite in favourites) {
            results.Add(db.GetMedia(new Media() {Id = favourite.MediaId})!);
        }
        return results;
    }
}
